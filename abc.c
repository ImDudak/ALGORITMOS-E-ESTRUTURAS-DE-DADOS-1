...
#include <stdlib.h> 
#include <locale.h>
#include <C:\Users\dudak\Desktop\Programim in C\teste3\abc.h>
...
void inserirOrd(ListaSimplesEnc *pList, int v) {
	Nodo *novo;	
	novo = (Nodo*)malloc(sizeof(Nodo));	//aloca memoria
	if (novo != NULL) {
		
		novo->info = v;
		Nodo *pAtu, *pAnt; //define ponteiros do nodo atual e anterior
		pAnt = NULL;	
		pAtu = pList->prim;	
